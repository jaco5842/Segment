...
app.timer('SubscribedToProductPW', {
    schedule: '0 0 2 * * *',
    handler: async (myTimer, context) => {
        ...
        const sqlFilePath = path.join(__dirname, 'sql', 'PW', 'subscribedToProduct.sql');
        ...
        try {
            ...
            for (const row of results) {
                ...
                await identify(context, company, row.bcid, row.email, row);

                // Call the track function if unsubscribed
                if (row.subscriptionStatus === 'unsubscribed') {
                    let messageId = process.env.testmode === 'true' ? row.event + '-' + row.invoiceNo + company + moment() : row.event + '-' + row.invoiceNo + company;                
                    await track(context, company, row.event, row.bcid, row.email, messageId, row.timestamp, row);
                    context.log("Unsubscribed Track Request sent");
                }
            }
